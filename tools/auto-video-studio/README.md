...
src/
  voices.js          preset giọng + edge-tts/gTTS (tự dự phòng)
  music.js           kho nhạc local + sync Google Drive
  scriptgen.js       nội dung → danh sách cảnh
  composition.js     cảnh → HTML HyperFrames + timeline
  pipeline.js        toàn bộ pipeline 1 video
...
| `PYTHON_BIN` | đường dẫn python (mặc định `python`) |
