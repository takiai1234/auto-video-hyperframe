...
Chạy **ngầm** (không phụ thuộc IDE, đóng IDE vẫn chạy): double-click `start.cmd`. Dừng: nút **⏻ Tắt tool** trên giao diện hoặc `stop.cmd`.

## Tính năng

- **Tự sinh nội dung từ chủ đề (OpenRouter / ChatGPT)**: bật “🤖 Tự sinh nội dung”, chỉ nhập **chủ đề** → AI viết kịch bản 2-3 phút. Nhập API key + model OpenRouter trên giao diện (mặc định `openai/gpt-4.1-mini`). Code: `src/openrouter.js`. Tắt AI thì dùng nội dung bạn dán.
...
- **Giọng đọc tiếng Việt - Vbee (API)** (một video dùng 1 giọng để **đồng nhất**): chất lượng cao, nhiều giọng. Nhập `app_id` + `access token` trong UI (hoặc env `VBEE_APP_ID`/`VBEE_TOKEN`), thêm các `voice_code` của gói bạn, bấm **Test**. Endpoint chỉnh được ở Base URL (mặc định `https://vbee.vn/api/v1/tts`). Code: `src/vbee.js` + `src/settings.js`.
...
- **Lịch sử task được lưu** ở `data/tasks.json`: còn nguyên khi tắt/mở lại tool, **tự xoá task cũ hơn 7 ngày**. Task đang chạy lúc tool tắt sẽ về trạng thái chờ để chạy lại.
- **Chạy ngầm**: chạy `start.cmd` (Windows) để tool chạy nền, **không phụ thuộc IDE** (đóng IDE tool vẫn chạy). Dừng bằng nút **⏻ Tắt tool** trên giao diện (hoặc `stop.cmd`).
...
| `CONCURRENCY` | số video chạy song song lúc khởi động (mặc định 2; chỉnh được 1-5 ngay trên giao diện, lưu vào `data/settings.json`) |
...
| `OPENROUTER_MODEL` | model OpenRouter (mặc định `openai/gpt-4.1-mini`) |
